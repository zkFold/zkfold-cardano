...
main = do
  ...
  IdentityCircuitContract x ps <- fromJust . decode <$> BL.readFile (path </> "test-data" </> "plonkupVerifierTx-contract-data.json")
  ...
  txin2 <- parseJsonToTxInInfoList [ Just $ plonkupVerifierTxCompiled setup ] <$> BL.readFile (assetsPath </> "utxo2.json")
  ...
  case dataE of
    ...
    Right (txIns, txRefs, addr1) -> do
      ...
      BS.writeFile (assetsPath </> "redeemerPlonkupVerifierTx.json") $ prettyPrintJSON $ dataToJSON proof
