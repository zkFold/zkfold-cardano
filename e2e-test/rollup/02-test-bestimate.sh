...
total2=$(cardano-cli query utxo --address $(cat ./keys/alice.addr) --testnet-magic 4 --out-file /dev/stdout | jq -r '. | to_entries[1].value.value.lovelace')
# fee=3000000
...
execUnits="($(($cpuB + $cpuM * $updateLength)), $(($memB + $memM * $updateLength)))"
