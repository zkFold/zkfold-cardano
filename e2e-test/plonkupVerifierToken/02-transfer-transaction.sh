...
cabal run plonkupVerifierToken-transfer-transaction -- $policyid

datum=$assets/datumPlonkupVerifierToken.cbor
...
cardano-cli conway transaction build \
    ...
    --out-file "$keypath/plonkupVerifierToken-transfer.txbody" \
    --tx-in $in \
...
cardano-cli conway transaction sign \
    --testnet-magic $magic \
    --tx-body-file "$keypath/plonkupVerifierToken-transfer.txbody" \
    --signing-key-file "$keypath/charles.skey" \
    --out-file "$keypath/plonkupVerifierToken-transfer.tx"

cardano-cli conway transaction submit \
    --testnet-magic $magic \
    --tx-file "$keypath/plonkupVerifierToken-transfer.tx"
...
echo ""
echo "transaction id: $(cardano-cli conway transaction txid --tx-file "$keypath/plonkupVerifierToken-transfer.tx")"
echo ""
