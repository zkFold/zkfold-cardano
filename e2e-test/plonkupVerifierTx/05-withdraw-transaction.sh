...
plonkupVerifierTx=$(cardano-cli conway transaction txid --tx-file "$keypath/plonkupVerifierTx.tx")#0
forwardingRewardReference=$(cardano-cli conway transaction txid --tx-file "$keypath/forwardingMint.tx")#0
base16=$(cardano-cli conway address info --address "$keypath/plonkupVerifierTx.addr" /dev/stdout | jq -r 'keys[0]')

forwardingRewardIn=$(cardano-cli conway transaction txid --tx-file "$keypath/plonkup-transfer.tx")#0

#---------------------------------- :redeemer: ---------------------------------

cabal run plonkupVerifierTx-withdraw-transaction -- $base16

plutusStakingScriptRedeemer=$assets/redeemerPlonkupVerifierTx.json
...
cardano-cli conway transaction build \
    ...
    --tx-in $in \
    --withdrawal "$(cat $keypath/plonkupVerifierTxStaking.addr) + 0 lovelace" \
    --withdrawal-plutus-script-v3 \
    --withdrawal-tx-in-reference "$assets/plonkupVerifierTx.plutus" \
