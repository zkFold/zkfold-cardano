...
policyid=$(cardano-cli conway transaction policyid --script-file "$assets/plonkupVerifierTx.plutus")
...
cabal run plonkupVerifierTx-transfer-transaction -- $policyid
...
cardano-cli conway transaction build \
    ...
    --out-file "$keypath/plonkupVerifierTx-transfer.txbody" \
    --tx-in $in \
...
cardano-cli conway transaction sign \
    --testnet-magic 4 \
    --tx-body-file "$keypath/plonkupVerifierTx-transfer.txbody" \
    --signing-key-file "$keypath/alice.skey" \
    --out-file "$keypath/plonkupVerifierTx-transfer.tx"

cardano-cli conway transaction submit \
    --testnet-magic 4 \
    --tx-file "$keypath/plonkupVerifierTx-transfer.tx"
...
echo ""
echo "transaction id: $(cardano-cli conway transaction txid --tx-file "$keypath/plonkupVerifierTx-transfer.tx")"
echo ""
