...
type PlonkNZ i n = Plonk i n 0 BLS12_381_G1 BLS12_381_G2 BuiltinByteString
...
mkSetup' :: forall i n . KnownNat n => SetupVerify (PlonkNZ i n) -> SetupBytes
mkSetup' PlonkupVerifierSetup {..} =
  let PlonkupCircuitCommitments {..} = commitments
  in SetupBytes
    { n          = fromIntegral (value @n)
...
mkInput' :: Input (PlonkNZ i n) -> InputBytes
mkInput' (PlonkupInput input) = F . convertZp . negate $ V.head input

mkProof' :: Proof (PlonkNZ i n) -> ProofBytes
mkProof' PlonkupProof {..} = ProofBytes
  { cmA_bytes     = convertG1 cmA
  , cmB_bytes     = convertG1 cmB
  , cmC_bytes     = convertG1 cmC
